/* Generated by Brace_UMD 0.2.8 */
(function() {
    var e = {
        e: typeof i !== 'undefined' && i || undefined,
        n: typeof o !== 'undefined' && o || undefined,
        i: typeof module !== 'undefined' && module || undefined,
        o: arguments[0] || undefined,
        r: {
            e: true,
            n: true,
            t: true,
            d: true
        },
        f: arguments[0] && arguments[0].f || arguments[1] && arguments[1].f,
        s: [ 'config', 'nextTick', 'version', 'jsExtRegExp', 'isBrowser', 's', 'toUrl', 'undef', 'defined', 'specified', 'onError', 'createNode', 'load', 'exec' ],
        u: [ 'amd', 'require' ],
        c: function() {
            if (e.i && !e.n) try {
                e.n = e.i.t('amdefine');
                for (var n in e.n) this[n] = e.n[n];
            } catch (e) {
                console.log(e.message);
            }
            var o = e.n || e.d;
            o.apply(o.prototype, arguments);
        },
        a: function() {
            if (e.i && !e.e) try {
                e.e = e.i.t('requirejs');
                for (var n in e.e) this[n] = e.e[n];
            } catch (e) {
                console.log(e.message);
            }
            var o = e.e || e.d;
            o.apply(o.prototype, arguments);
        }
    };
    for (var n in e.u) e.c.__defineGetter__(e.u[n], function(n) {
        if (e.i) try {
            e.n = e.i.t('amdefine')(module);
            for (var o in e.n) this[o] = e.n[o];
        } catch (e) {
            console.log(e.message);
        }
        return e.n[n];
    }.bind(null, e.u[n]));
    for (var n in e.s) e.a.__defineGetter__(e.s[n], function(n) {
        if (e.i) try {
            e.e = e.i.t('requirejs');
            for (var o in e.e) e.a[o] = e.e[o];
        } catch (e) {
            console.log(e.message);
        }
        return e.e[n];
    }.bind(null, e.s[n]));
    e.d = e.o && function(e, n, o, i) {
        if (e && e.constructor === Array && this.i) {
            var t = e, r = dependency, f = o, s = i;
            i = s;
            o = f;
            dependency = t;
            e = this.i.l;
        }
        if (typeof e !== 'string') console.log('The global native Object is attempted to be used but the module does not supply an id parameter. Skipping loading of the module.'); else if (n.every(function(n, o) {
            return n in this.o || !!console.log('The dependency', n, 'is not loaded into the factory yet. Skipping loading of the module', e);
        }, this)) this.o[e] = o.apply(o.prototype, n.map(function(e, n) {
            return this.o[e];
        }, this));
    }.bind(e);
    if (e.f) if (!(e.f in e.r)) {
        console.log('The forced type', e.f, 'specified as an option is not supported by Brace UMD. Supported types are', Object.keys(e.r));
    } else if (!e[e.f]) {
        console.log('The forced type', e.f, 'is not available.');
    } else {
        console.log('Forcing use of the definition type', e.f);
        e.e = e.t = e.n = e.d = e[e.f];
    }
    var o = e.n || e.c;
    var i = e.e || e.a;
    var t = e.i && e.i.t || i;
    o('entry', [], function(e) {
        console.log(this);
        console.log(e);
    });
})(this);